    ...
    pub(crate) fn inactive(&mut self) {
        ...
        for cell in &mut self.cells {
            cell.style = match cell.style {
                ...
                Style::SelectedMatchingBracket => Style::Selection,
                other => other,
            };
        }
    }
        ...
        for y in 0..self.back.height {
            for x in 0..self.back.width {
                ...
                if last_style != Some(cell.style) {
                    ...
                    if last_style.is_some_and(Style::crossed_out) != cell.style.crossed_out() {
                        queue!(
                            self.output,
                            SetAttribute(if cell.style.crossed_out() {
...
mod tests {
    ...
    #[test]
    fn matching_partner_is_yellow_bold_and_underlined_without_changing_the_cursor() {
        assert_eq!(
        ...
        frame.put(0, 0, "(", Style::PrimaryCursor(None));
        ...
        assert!(output.contains("\x1b[27mx"));
    ...
    fn markdown_attributes_are_enabled_and_cleared_between_cells() {
        ...
        for (highlight, enabled, disabled) in [
            ...
            (Highlight::Strikethrough, "\x1b[9m", "\x1b[29m"),
