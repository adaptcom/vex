    ...
    fn from_capture(mut name: &str) -> Option<Self> {
        loop {
            let highlight = match name {
                ...
                | "import" | "media" | "supports" | "keyframes" | "storageclass" => Self::Keyword,
                "type" => Self::Type,
                "constructor" => Self::Constructor,
                "tag" => Self::Tag,
                "namespace" | "module" => Self::Namespace,
                "function" | "method" => Self::Function,
                "constant" | "number" | "float" | "boolean" => Self::Constant,
...
"#,
            ...
            (
                Language::Sql,
                "-- comment\nCREATE TEMPORARY TABLE users (id INTEGER, name TEXT, active BOOLEAN);\n/* block comment */\nSELECT email, COUNT(*) AS total FROM users WHERE id >= 42 AND name = 'hello' AND active = TRUE;\n",
                &[
                    ("-- comment", Comment),
                    ("CREATE", Keyword),
                    ("TEMPORARY", Keyword),
                    ...
                    ("COUNT", Function),
                    (">=", Operator),
                    // The bundled query's Lua-style numeric predicates do not
                    // match under Tree-sitter's Rust regex engine.
                    ("42", String),
                    ("hello", String),
                    ("TRUE", Constant),
                ],
            ...
            (
                Language::Sql,
                "with recent as (select id from users where score > 1.5) select recent.id from recent;\n",
                &[
                    ("with", Keyword),
                    ("select", Keyword),
                    ("id", Property),
